...
Hi everyone, I am Yingtao Tian from Sakana AI. This talk is about CEDAR,
...
Here is the whole pitch on one slide. On the right is World Dynamics,
...
Watch the three colored pairs: each objective in the sentence, population,
resources, pollution, connects to its defining equation in the code. And now
the gray arrows: those are the real dependencies between variables in this
model, and this flickering web is exactly why hand-editing such a system is
so hard. One change cascades everywhere.
...
Formally we use ODEs integrated with first-order Euler, as in the formula.

The catch, and this is a central open problem in ALife: predicting how a
feedback structure produces emergent behavior is very hard. So designing a
system to exhibit a target behavior is even harder.

## 4. The problem (0:45 / cum 3:05)

The bottleneck is double. Scientifically, structure to behavior is opaque,
...
The opportunity: LLM agents are strong at code, planning, and reasoning,
and Monte Carlo tree search can give their search structure. CEDAR puts
those two together.

## 5. CEDAR in one picture (1:15 / cum 4:20)

You give CEDAR two things: a goal in natural language, and an initial
system written in a restricted subset of Python. I will say more about that
...
PRESS. And each step has the same shape. A: select a node and a strategy.
B: expand. The LLM Editor writes a modified system, we run it, and an LLM
Judge scores the result against the goal.
...
PRESS. And because it is runnable, executing it traces every variable at
...
breakthrough to conservative. In our runs: Claude Sonnet 4.5 and GPT-5.1 as
backends, twenty selections times five candidates, so one hundred
expansions per search.

The point of the branching: many good, different systems stay alive at
once. Selection pressure without collapse. And in ablations, MCTS beats
linear search.

## 9. Results: the goal is self-conflicting (0:50 / cum 7:45)

Now results. First experiment: that World Dynamics goal from the opening.
Why is it hard? The hand-designed model already sits near a Pareto
...
PRESS. So the real test is improving all three subgoals jointly, and that
is exactly what we ask CEDAR to do.
...
And here is what comes back. On the left, the actual search tree from a
...
What I find most interesting: run it with Claude and with GPT-5.1, and you
get structurally distinct solutions. One leans population-first, the other
resource-first. One vague sentence in, a portfolio of qualitatively
different sustainable systems out.
...
only fits parameters. Strictly more prior structure than CEDAR receives.

The table: without formulae Optuna is far off, L1 around twenty-nine.
With the full equations it reaches 3.71. CEDAR, with no formulae at all,
gets 3.29 with Claude, and 2.22 with GPT-5.1, which also wins on dynamic
time warping. The plot shows why: weak baselines collapse to near-constant
populations, while CEDAR tracks the peaks and dips of the ground truth.
...
satisfy the goal along visibly distinct trajectories. That is a sensitivity
...
with up to three retries, and crashed or NaN runs are scored low so the
...
stochastic; across ten random seeds its volatility is intrinsic, and
CEDAR's fit sits inside that band. And a second full-formulae Optuna run
does not close the gap: 4.26 versus 3.71 on L1, both above CEDAR's 2.22.
Parameter fitting with the true equations still trails, because of the
noise floor and a sensitive nonlinear landscape. Full Judge and Editor
transcripts are released, and on human inspection the scores and the
reasoning align.

## 16. Limits, and what comes next (0:45 / cum 13:40)

Honestly stated limits. The Judge scores what a same-class LLM edited, so
there is a circularity risk for abstract goals. We report trends, not tight
statistics, with two systems studied in depth. The rationales make the
search inspectable, but they are not yet a verified causal account of
emergence. And convergence guarantees for MCTS with LLMs remain open.

Next: systematic benchmarks against classical evolutionary and
...
Ask for the behavior, and let the agents grow the system. Thank you, and I
